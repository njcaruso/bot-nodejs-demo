...
BotControllers.prototype.start = function() {
  ...
  controller.hears(['bored'], 'direct_message,direct_mention,mention', function(bot, message) {
...
function searchYouTube(bot, message, result, cbSearchYouTube) {
  ...
  youtube.search(youtubeQuery, function(errSearchingYoutube, foundResults) {
    ...
    foundResults.items.forEach(function(item) {
      ...
      try {
        viewCount = item.statistics.viewCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        likeCount = item.statistics.likeCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      } catch (e) {
        console.log('error parsing view or like, e = ', e);
      }
...
function startNewConversation(bot, message) {
  ...
  bot.createConversation(message, function(errCreatingConv, convo) {
    ...
    askAI(bot, 'Let\'s watch some YouTube!  Give me a topic.', convo);
    convo.activate();
  });
}
